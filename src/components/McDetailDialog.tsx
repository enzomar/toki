/**
 * McDetailDialog — Monte Carlo Simulation Details Modal
...
export function McDetailDialog({ open, onClose, mcReport, currency = 'EUR' }: Props) {
    ...
    <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
      <DialogTitle sx={{ pb: 1 }}>
        <Stack direction="row" sx={{ justifyContent: 'space-between', alignItems: 'center' }}>
          <Typography variant="h6" sx={{ fontWeight: 700 }}>Monte Carlo Simulation Details</Typography>
          ...
          <Stack spacing={3}>
            ...
            <Grid container spacing={2}>
              ...
              <Grid size={3}>
                <Paper variant="outlined" sx={{ p: 1.5, textAlign: 'center' }}>
                  ...
                  <Typography variant="h6" sx={{ fontWeight: 700, color: mcReport.confidence_score > 0.8 ? 'success.main' : mcReport.confidence_score > 0.6 ? 'warning.main' : 'error.main' }}>{Math.round(mcReport.confidence_score * 100)}%</Typography>
                </Paper>
              </Grid>
            </Grid>
            ...
            <Paper variant="outlined" sx={{ p: 2 }}>
              ...
              <Stack direction="row" sx={{ mt: 1.5, justifyContent: 'space-between' }}>
                <Typography variant="caption" color="text.secondary">Std deviation: {formatMetricNumber(Math.round(Math.sqrt(mcReport.variance_tokens)))} tokens</Typography>
              ...
              <Grid container spacing={1}>
                {[
                  ...
                  <Grid key={label} size={3}>
                    <Box sx={{ textAlign: 'center', p: 1, bgcolor: bg, borderRadius: 1 }}>
            ...
            <Paper variant="outlined" sx={{ p: 2 }}>
              ...
              <Grid container spacing={2}>
                <Grid size={4}><Typography variant="caption" color="text.secondary">Deterministic tokens</Typography><Typography variant="body2" sx={{ fontWeight: 600 }}>{formatMetricNumber(mcReport.deterministic_tokens_per_conv)}/conv</Typography></Grid>
                <Grid size={4}><Typography variant="caption" color="text.secondary">MC expected tokens</Typography><Typography variant="body2" sx={{ fontWeight: 600 }}>{formatMetricNumber(mcReport.tokens_expected_per_conv)}/conv</Typography></Grid>
                <Grid size={4}><Typography variant="caption" color="text.secondary">Alignment ratio</Typography><Typography variant="body2" sx={{ fontWeight: 600, color: mcReport.alignment_ok ? 'success.main' : 'warning.main' }}>×{mcReport.alignment_ratio.toFixed(3)} {mcReport.alignment_ok ? '✓' : '⚠'}</Typography></Grid>
                <Grid size={4}><Typography variant="caption" color="text.secondary">Input tokens/conv</Typography><Typography variant="body2" sx={{ fontWeight: 600 }}>{formatMetricNumber(mcReport.input_tokens_per_conv)}</Typography></Grid>
                <Grid size={4}><Typography variant="caption" color="text.secondary">Output tokens/conv</Typography><Typography variant="body2" sx={{ fontWeight: 600 }}>{formatMetricNumber(mcReport.output_tokens_per_conv)}</Typography></Grid>
                <Grid size={4}><Typography variant="caption" color="text.secondary">Variance</Typography><Typography variant="body2" sx={{ fontWeight: 600 }}>{formatMetricNumber(Math.round(mcReport.variance_tokens))} tok²</Typography></Grid>
              </Grid>
            </Paper>
          </Stack>
        ) : (
          <Typography color="text.secondary">Run a simulation to see detailed results.</Typography>
        )}
      </DialogContent>
    </Dialog>
  )
